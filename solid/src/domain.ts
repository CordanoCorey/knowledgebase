...
const TAGS_BY_ID = new Map(REFERENT_TAG_FIXTURES.map((tag) => [tag.id, tag]));
...
export const CALENDAR_EVENTS = [
  {
    contextHref: "/goto/grade-9-church-history",
    contextLabel: "Grade 9 Church History",
    day: 12,
    groupLabel: "Upper School",
    id: "seminar-crusades",
    locationLabel: "Room 204",
    status: "confirmed" as const,
    timeLabel: "9:00 AM",
    title: "Crusades source seminar",
  },
  {
    ...
    groupLabel: "Bible",
    id: "daniel-memory",
    locationLabel: "Chapel",
    status: "draft" as const,
    timeLabel: "10:30 AM",
    title: "Daniel memory work",
  },
  {
    contextHref: "/goto/americas-founding-250",
    contextLabel: "250th Celebration",
    day: 24,
    groupLabel: "Community",
    id: "founding-planning",
    locationLabel: "Library",
    status: "confirmed" as const,
    timeLabel: "2:00 PM",
    title: "Founding celebration planning",
  },
];

export const TODAY_AGENDA_ITEMS = [
  {
    contextHref: "/scripture/joshua-1-6-9",
    contextLabel: "Joshua 1:6-9",
    detail: "Gather answers and quotes for morning assembly.",
    groupLabel: "Faculty",
    id: "assembly-courage",
    statusLabel: "Open",
    timeLabel: "8:15 AM",
    title: "Courage assembly preparation",
  },
  {
    contextHref: "/goto/first-crusade",
    contextLabel: "First Crusade",
    detail: "Review student questions and add one source note.",
    groupLabel: "History",
    id: "crusade-review",
    statusLabel: "Needs contribution",
    timeLabel: "11:00 AM",
    title: "History seminar follow-up",
  },
];
...
export function getActiveTagsFromRoute(routeState: Pick<RouteState, "pathname" | "search">): ActiveTag[] {
  ...
  if (pathname.startsWith("/scripture/")) {
    return [resolveBiblePassageTag(decodePathSegment(pathname.slice("/scripture/".length)))];
  }
  if (pathname.startsWith("/goto/")) {
    return [resolveTag(decodePathSegment(pathname.slice("/goto/".length)))];
  }
  if (pathname.startsWith("/organizations/")) {
    return [resolveTag(decodePathSegment(pathname.split("/")[2] ?? SAMPLE_ORG_ID))];
  }
  return [];
}
...
function resolveBiblePassageTag(value: string): ActiveTag {
  ...
  const fixture = TAGS_BY_ID.get(id);
  return fixture ?? biblePassageTag(id, labelFromTagId(id).replace(/\b(\d+)\b/g, "$1"));
}
